-- Durable, transcript-free replay ledger for service-authenticated PSTN turns.
-- The request digest proves idempotency without retaining what the caller said.
-- Completed payloads contain only Civya-approved public speech and safe metadata.

create table private.phone_turn_results (
...
create index phone_turn_results_lease_idx
...
begin
  ...
  if v_turn.state = 'processing' and v_turn.processing_lease_expires_at > now() then
    return jsonb_build_object(
      ...
      'retryAfterSeconds', greatest(1,
        ceil(extract(epoch from (v_turn.processing_lease_expires_at - now())))::integer)
    );
  end if;
  ...
  if v_turn.attempt_count >= 8 then
    update private.phone_turn_results set
      ...
      processing_lease_expires_at = null, error_code = 'phone_turn_attempts_exhausted',
      updated_at = now()
...
begin
  ...
  if jsonb_typeof(p_response_payload) <> 'object'
     or octet_length(p_response_payload::text) > 16000 then
  ...
  update private.phone_turn_results set
    ...
    processing_lease_expires_at = null, error_code = null, updated_at = now()
  where id = p_turn_id and state = 'processing'
...
grant execute on function public.civya_service_claim_phone_turn(uuid, text, text, text, text, text, integer)
  to service_role;
grant execute on function public.civya_service_finish_phone_turn(uuid, text, uuid, jsonb)
  to service_role;
grant execute on function public.civya_service_fail_phone_turn(uuid, text, uuid, text)
  to service_role;

comment on table private.phone_turn_results is
  'Transcript-free idempotency and replay ledger for approved public PSTN responses.';
